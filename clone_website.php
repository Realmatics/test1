...
class WebsiteCloner {
    ...
    private $maxPages;
    private $maxDepth;
    private $stats = ['pages' => 0, 'css' => 0, 'js' => 0, 'images' => 0, 'fonts' => 0, 'ajax' => 0, 'errors' => 0];
    ...
    public function clone_site() {
        ...
        while (!empty($this->pageQueue) && count($pagesToProcess) < $this->maxPages) {
            ...
            $html = $this->prefetchAjaxInjectedContent($html, $normalized);
        ...
        $this->log[] = "AJAX-Vorabruf (Grids o.ä.): " . $this->stats['ajax'];
    ...
    /**
     * Lädt serverseitig Inhalte nach, die im Original per AJAX erscheinen (z. B. WPBakery vc_media_grid).
     */
    private function prefetchAjaxInjectedContent($html, $pageUrl) {
        if (stripos($html, 'data-vc-grid-settings') === false) {
            return $html;
        }

        if (!preg_match_all(
            '/<div(?=[^>]*\bvc_grid-container\b)(?=[^>]*data-vc-grid-settings="([^"]+)")[^>]*>/is',
            $html,
            $matches,
            PREG_OFFSET_CAPTURE
        )) {
        ...
        $n = count($matches[0]);
        for ($i = 0; $i < $n; $i++) {
            $fullOpenTag = $matches[0][$i][0];
            $openStart = $matches[0][$i][1];
            $settingsRaw = $matches[1][$i][0];
            $openTagEnd = $openStart + strlen($fullOpenTag);

            $postId = null;
            $nonce = null;
            $ajaxUrl = null;
            if (preg_match('/\bdata-vc-post-id="([^"]*)"/i', $fullOpenTag, $m)) {
                $postId = $m[1];
            }
            if (preg_match('/\bdata-vc-public-nonce="([^"]*)"/i', $fullOpenTag, $m)) {
                $nonce = $m[1];
            }
            if (preg_match('/\bdata-vc-request="([^"]*)"/i', $fullOpenTag, $m)) {
                $ajaxUrl = html_entity_decode($m[1], ENT_QUOTES | ENT_HTML5, 'UTF-8');
            }
            if ($postId === null || $postId === '' || $nonce === null || $nonce === '') {
                continue;
            }
            if (!$ajaxUrl) {
            ...
            $bounds = $this->findMatchingDivBounds($html, $openTagEnd);
            if (!$bounds) {
                continue;
            }
            $inner = $bounds['inner'];
            $innerStripped = trim(preg_replace('/<!--.*?-->/s', '', $inner));
            if (strlen($innerStripped) > 80) {
                continue;
            }

            $settingsJson = html_entity_decode($settingsRaw, ENT_QUOTES | ENT_HTML5, 'UTF-8');
            $settings = json_decode($settingsJson, true);
            if (!is_array($settings) || empty($settings['tag'])) {
                continue;
            }

            $fragment = $this->fetchWpbVcGridAjax($ajaxUrl, $pageUrl, $postId, $nonce, $settings);
            if (!$fragment) {
                continue;
            }

            $fragment = preg_replace('/<link\b[^>]*rel\s*=\s*[\'"]stylesheet[\'"][^>]*>/i', '', $fragment);
            $newBlock = $fullOpenTag . $fragment . '</div>';
            $replacements[] = [
                'start' => $openStart,
                'end' => $bounds['outerEnd'],
                'html' => $newBlock,
            ...
            $this->stats['ajax']++;
            $this->log[] = '  ⚡ AJAX-Grid eingefügt (WPBakery): ' . ($settings['shortcode_id'] ?? '?');
        }
        ...
        foreach ($replacements as $r) {
            $html = substr($html, 0, $r['start']) . $r['html'] . substr($html, $r['end']);
        }
        return $html;
    }

    private function findMatchingDivBounds($html, $afterOpenTag) {
        $depth = 1;
        $pos = $afterOpenTag;
        $len = strlen($html);
        while ($depth > 0 && $pos < $len) {
            if (!preg_match('/<\s*div\b|<\s*\/\s*div\s*>/i', $html, $m, PREG_OFFSET_CAPTURE, $pos)) {
                return null;
            }
            $tagStart = $m[0][1];
            $tag = $m[0][0];
            if (stripos($tag, '</') === 0) {
                ...
                if ($depth === 0) {
                    return [
                        'inner' => substr($html, $afterOpenTag, $tagStart - $afterOpenTag),
                        'outerEnd' => $tagEnd,
                    ];
                }
                $pos = $tagEnd;
                ...
                $depth++;
                $pos = $tagStart + strlen($tag);
            }
        }
        return null;
    }

    private function fetchWpbVcGridAjax($ajaxUrl, $refererUrl, $postId, $nonce, array $settings) {
        $postFields = [
            'action' => 'vc_get_vc_grid_data',
            'vc_action' => 'vc_get_vc_grid_data',
            'tag' => $settings['tag'],
            'vc_post_id' => (string)$postId,
            '_vcnonce' => $nonce,
        ];
        ...
        $body = $this->postUrlForm($ajaxUrl, $postFields, $refererUrl);
        ...
        if ($trim !== '' && ($trim[0] === '{' || $trim[0] === '[')) {
            ...
            if (is_array($j) && isset($j['status']) && stripos($j['status'], 'nothing') !== false) {
                return null;
            }
        }
        if (stripos($body, 'vc_grid-item') === false && stripos($body, 'vc_pageable') === false) {
            return null;
        }
        return $body;
    }

    private function postUrlForm($url, array $fields, $refererUrl) {
        if (!$url) {
            return null;
        }
        $ch = curl_init();
